...
(define (pitchlist->lily pitches durations) 
   "Return a seq-mus object according to the duration(s) or pitch(es) One of the objects must be list. single-octave"
...
(define (durlist->spacers durations) 
   "Return a seq-mus object according to the duration(s) or pitch(es) One of the objects must be a list. single-octave"
   (make-sequential-music
    (map (lambda (i)
           (let* (
...
(define (add-slur seq-mus-arg)
  (if (equal? (ly:music-property seq-mus-arg 'name) 'SequentialMusic)
      (let ((lst (ly:music-property seq-mus-arg 'elements)))
	(cond
	 ((or (null? lst) (null? (cdr lst))
	      (null? (cdr (map (lambda (x) (ly:music-property x 'elements)) lst)))) seq-mus-arg)
	 (else
	  (let*
	      ((first-el (first lst))
	       (last-el (last lst))
...
(define (add-articulations seq-mus-arg articulation-type-arg)
  ...
  (let
    ...
    (begin
      ...
      (make-sequential-music lst)))
  (add-articulation-to-note seq-mus-arg articulation-type-arg)
  ))
